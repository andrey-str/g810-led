...
bool Keyboard::attach(unsigned short lg_pid) {
    ...
    m_dev_handle = libusb_open_device_with_vid_pid(m_ctx, 0x046d, lg_pid);
    if (m_dev_handle == NULL) {
        std::cout << "Unable to open device w/pid 0x" << std::hex << lg_pid << std::endl;
        libusb_exit(m_ctx);
        m_ctx = NULL;
        return false;
    }
    if (libusb_kernel_driver_active(m_dev_handle, 1) == 1) {
        libusb_detach_kernel_driver(m_dev_handle, 1);
        m_isKernellDetached = true;
    }
    r = libusb_claim_interface(m_dev_handle, 1);
    if (r < 0) return false;
    m_isAttached = true;
    return true;
}
...
bool Keyboard::detach() {
    ...
    int r = libusb_release_interface(m_dev_handle, 1);
    if (r != 0) {
        std::cerr << "Failed to release libusb interface"<<std::endl;
        return false;
    }

    if (m_isKernellDetached) {
        if(LIBUSB_SUCCESS != libusb_attach_kernel_driver(m_dev_handle, 1)){
            std::cerr << "Failed to re-attach kernel driver" << std::endl;
        }
    ...
    std::cout << "Device was detached" << std::endl;
...
bool Keyboard::commit() {
    ...
    bool retval = sendDataInternal(data, 20);
...
bool Keyboard::parseKey(std::string key, KeyAddress &keyAddress) {
    ...
    else if (key == "close_bracket" || key == "closebracket" || key == "¨" || key == "!")
    ...
    else if (key == "print_screen" || key == "printscreen" || key == "printscr" || key == "print")
        parsedKey = Key::print_screen;
    else if (key == "scroll_lock" || key == "scrolllock") parsedKey = Key::scroll_lock;
    else if (key == "pause_break" || key == "pausebreak" || key == "pause" || key == "break")
...
bool Keyboard::parseColor(std::string color, KeyColors &colors) {
    ...
    if (color.length() != 6) {
        std::cout << "Color string seems to be invalid: " << color << std::endl;
        return false;
    }
...
bool Keyboard::sendDataInternal(unsigned char *data, int data_size) {
    ...
    int r;
    if (data_size > 20)
        r = libusb_control_transfer(m_dev_handle, 0x21, 0x09, 0x0212, 1, data, data_size, 2000);
    else
        r = libusb_control_transfer(m_dev_handle, 0x21, 0x09, 0x0211, 1, data, data_size, 2000);
...
bool Keyboard::setKeysInternal(KeyAddressGroup addressGroup, KeyValue keyValues[], int keyValueCount) {
    ...
    bool retval = sendDataInternal(data, data_size);
    delete[] data;
    return retval;
}
